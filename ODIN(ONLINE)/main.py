                    ...
                    async for message in client_websocket: 
                        ...
                        if "realtime_input" in data:
                            for chunk in data["realtime_input"]["media_chunks"]:
                                mime_type = chunk.get("mime_type", "unknown")
                                encoded_data = chunk.get("data", "")
                                if not encoded_data:
                                    print(f"Received empty data for mime_type: {mime_type}")
                                else:
                                    print(f"Data received for {mime_type}: Length = {len(encoded_data)}")
                                    if mime_type == "audio/pcm":
                                        print(f"PCM Base64 Data (truncated): {encoded_data[:50]}...")
                                    elif mime_type == "image/jpeg":
                                        print(f"JPEG Base64 Data (truncated): {encoded_data[:50]}...")

                                # Forward the chunk to Gemini if it contains valid data
                                if mime_type in ["audio/pcm", "image/jpeg"] and encoded_data:
                                    await session.send({"mime_type": mime_type, "data": encoded_data})
                                else:
                                    print(f"Skipping invalid or empty chunk for mime_type: {mime_type}")
